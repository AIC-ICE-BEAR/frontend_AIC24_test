      ...
      <div className={`overflow-y-auto flex-col ${ImageSimformVisible ? 'h-1/2' : 'h-full'} divide-solid transition-all duration-300`}>
        ...
        {ImageSimformVisible && (
            <div className="h-1/2 w-full">
              ...
              {renderImagesSimilarityForm(ImageSimiResult,setImageSimformVisible , selectedImage, handleImageClick, handleDoubleClick, handleClickImgSim,ClipConfig,  setImageSimiResult)}
            </div>
          )}
        <label>Result</label>
          ...
          <div className="overflow-y-auto h-screen" ref={scrollContainerRef}>
            <div className="grid grid-cols-4 gap-2">
              {displayResult.map((item, index) => (
                ...
                >
                  <img
                    src={`${process.env.REACT_APP_IMAGE_PATH}/${item.video_name.slice(0, 3)}/${item.video_name}/${item.keyframe_id}.jpg`}
              ...
              <div key={index} className="border border-gray-300 p-1 bg-white my-2">
                <div className="grid grid-cols-5 gap-2 " >
                  {item.keyframe_id.map((key_id, index) => (
                    <div> 
                      <img
                      src={`${process.env.REACT_APP_IMAGE_PATH}/${item.video_name.slice(0, 3)}/${item.video_name}/${key_id}.jpg`}
